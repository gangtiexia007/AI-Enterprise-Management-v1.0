    ...
    <div className="flex h-screen bg-surface-0 text-txt-1 antialiased">
      ...
      >
        <div className="flex h-12 items-center px-4 border-b border-border">
          <span className="text-sm font-semibold text-accent-light tracking-tight">千方百计AI</span>
        </div>

        <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-4 scrollbar-thin">
          {NAV_GROUPS.map((group) => (
            <div key={group.group}>
              <div className="px-3 py-1 text-[10px] uppercase tracking-[0.08em] text-txt-4 font-medium">
                {group.group}
              </div>
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-2.5 rounded-btn px-3 py-1.5 text-[13px] font-medium transition-colors ${
                      isActive
                        ? 'bg-[rgba(255,255,255,0.06)] text-txt-1'
                        : 'text-txt-3 hover:bg-[rgba(255,255,255,0.03)] hover:text-txt-2'
                    }`
                  }
                >
        ...
        <div className="border-t border-border px-3 py-2.5">
          <div className="flex items-center gap-2 min-w-0">
            <div className="h-7 w-7 rounded-full bg-accent/20 text-accent-light flex items-center justify-center text-[11px] font-semibold flex-shrink-0">
              老
            </div>
            <div className="min-w-0">
              <div className="text-[13px] font-medium text-txt-2 truncate">老板</div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="flex h-12 items-center justify-between border-b border-border bg-surface-1 px-4 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-[14px] font-medium text-txt-2 tracking-tight">{currentPageTitle()}</h1>
          </div>
          <button
            onClick={() => setChatOpen(true)}
            className="flex items-center gap-1.5 px-3 py-1 text-[13px] text-accent-light border border-border rounded-btn hover:bg-[rgba(255,255,255,0.03)] transition-colors"
          >
