...
export default function Modal({ open, onClose, title, children, width = 'max-w-md', footer }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20" onClick={onClose}>
      <div className={`bg-surface-1 rounded-panel border border-border shadow-xl w-full ${width} mx-4 max-h-[90vh] overflow-y-auto scrollbar-thin`}
        ...
        {footer && (
          <div className="flex justify-end gap-2 px-5 py-3 border-t border-border-subtle">{footer}</div>
        )}
      </div>
    </div>
  );
