...
export default function KPI() {
  ...
  const save = async () => {
    setSaving(true);
    try { await createKPI(form); setModalOpen(false); load(); }
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <StatCard label="平均分" value={avgScore} accent="text-emerald" subtitle="全员平均绩效分" />
        <StatCard label="考核员工" value={summary.length} accent="text-accent-light" />
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          <BarChart3 className="w-10 h-10 mx-auto mb-3 text-txt-4" />
          <div className="text-[14px] font-medium text-txt-3">暂无 KPI 数据</div>
        </div>
      ) : (
        <div className="space-y-4">
          {Object.entries(grouped).map(([name, items]) => (
            <div key={name} className="rounded-card border border-border overflow-hidden">
              <div className="border-b border-border px-4 py-2.5 flex items-center justify-between">
                <h3 className="text-[13px] font-semibold text-txt-1">{name}</h3>
                <span className="text-[11px] text-txt-4">{items.length} 项指标</span>
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-full text-[13px]">
                  <thead>
                    <tr className="text-left text-[11px] text-txt-4 border-b border-border">
                  ...
                  <tbody className="divide-y divide-border">
                    {items.map((r) => (
                      <tr key={r.id} className="hover:bg-[rgba(255,255,255,0.02)] transition-colors">
                        ...
                        <td className="px-4 py-2.5"><span className="inline-flex px-1.5 py-0.5 rounded-micro text-[11px] bg-[rgba(255,255,255,0.06)] text-txt-3">{r.period || '-'}</span></td>
                        ...
                        <td className="px-4 py-2.5 text-right">
                          <button onClick={() => handleDelete(r.id)} className="text-[12px] text-red-400/70 hover:text-red-400 transition-colors">删除</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="新增 KPI"
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-[12px] font-medium text-txt-3 mb-1.5">员工 <span className="text-red-400">*</span></label>
          ...
          <div>
            <label className="block text-[12px] font-medium text-txt-3 mb-1.5">指标名称 <span className="text-red-400">*</span></label>
            <input value={form.metric_name} onChange={e => setForm({...form, metric_name: e.target.value})} className={inputCls} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[12px] font-medium text-txt-3 mb-1.5">考核周期</label>
              <input value={form.period} onChange={e => setForm({...form, period: e.target.value})} placeholder="2024-Q1" className={inputCls} />
            </div>
            <div>
              <label className="block text-[12px] font-medium text-txt-3 mb-1.5">权重</label>
              <input type="number" step="0.1" value={form.weight} onChange={e => setForm({...form, weight: Number(e.target.value)})} className={inputCls} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
            ...
            <div>
              <label className="block text-[12px] font-medium text-txt-3 mb-1.5">实际值</label>
              <input type="number" value={form.actual_value} onChange={e => setForm({...form, actual_value: Number(e.target.value)})} className={inputCls} />
            </div>
          </div>
        </div>
      </Modal>
