  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard label="进行中" value={stats.in_progress} accent="text-amber-400" icon={<Clock className="w-4 h-4" />} />
        <StatCard label="已逾期" value={stats.overdue} accent="text-red-400" icon={<AlertTriangle className="w-4 h-4" />} />
        <StatCard label="已完成" value={stats.done} accent="text-emerald" icon={<CheckCircle className="w-4 h-4" />} />
      </div>
                  ...
                  <tr key={t.id} className="hover:bg-[rgba(255,255,255,0.02)] transition-colors">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-3">
                        {t.status === 'pending' && (
                          <button onClick={() => handleDispatch(t.id)} className="text-[12px] text-accent-light hover:text-accent-hover transition-colors">派发</button>
                        )}
                        <button onClick={() => openEdit(t)} className="text-[12px] text-txt-4 hover:text-txt-2 transition-colors">编辑</button>
                        <button onClick={() => handleDelete(t.id)} className="text-[12px] text-red-400/70 hover:text-red-400 transition-colors">删除</button>
                      </div>
                    </td>
                  </tr>
                ))}
                {tasks.length === 0 && (
                  <tr><td colSpan={6} className="px-4 py-12 text-center text-txt-4 text-[13px]">暂无任务数据</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? '编辑任务' : '创建任务'}
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-[12px] font-medium text-txt-3 mb-1.5">任务标题 <span className="text-red-400">*</span></label>
            <input value={form.title} onChange={e => setForm({...form, title: e.target.value})} className={inputCls} />
          </div>
          <div>
            <label className="block text-[12px] font-medium text-txt-3 mb-1.5">描述</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-[12px] font-medium text-txt-3 mb-1.5">优先级</label>
              <select value={form.priority} onChange={e => setForm({...form, priority: e.target.value as 'normal' | 'high' | 'urgent'})} className={inputCls}>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-[12px] font-medium text-txt-3 mb-1.5">关联目标</label>
              <select value={form.goal_id || ''} onChange={e => setForm({...form, goal_id: e.target.value ? Number(e.target.value) : undefined})} className={inputCls}>
