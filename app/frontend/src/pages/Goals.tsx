...
function GoalCard({ goal, allGoals, onEdit, onDelete, depth = 0 }: {
  ...
  return (
    <div className={depth > 0 ? 'ml-6 mt-2' : ''}>
      <div className="rounded-card border border-border bg-[rgba(255,255,255,0.02)] p-4 hover:bg-[rgba(255,255,255,0.03)] transition-colors">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-2 flex-1 min-w-0">
            {children.length > 0 && (
              <button onClick={() => setExpanded(!expanded)} className="mt-0.5 text-txt-4 hover:text-txt-2 transition-colors">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-2.5 flex items-center gap-2">
                <div className="flex-1 h-1 rounded-full bg-[rgba(255,255,255,0.06)] overflow-hidden max-w-xs">
                  <div className={`h-full rounded-full transition-all duration-500 ${
                    progress >= 80 ? 'bg-emerald' : progress >= 50 ? 'bg-accent-light' : 'bg-amber-400'
                  }`} style={{ width: `${Math.min(progress, 100)}%` }} />
                </div>
                <span className="text-[12px] text-txt-3 tabular-nums w-20 text-right">
          ...
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <button onClick={() => onDelete(goal.id)} className="text-[12px] text-red-400/70 hover:text-red-400 transition-colors">删除</button>
          </div>
        </div>
      </div>
...
export default function Goals() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const g = await getGoals(); setGoals(Array.isArray(g) ? g : []); }
    catch { setGoals([]); }
    setLoading(false);
  }, []);
  ...
  const save = async () => {
    setSaving(true);
    try { if (editingId) await updateGoal(editingId, form); else await createGoal(form); setModalOpen(false); load(); }
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <StatCard label="平均达成率" value={`${avgProgress}%`} accent="text-emerald" progress={avgProgress} />
        <StatCard label="进行中" value={allFlat.filter(g => g.status === 'active').length} accent="text-amber-400" />
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          <Target className="w-10 h-10 mx-auto mb-3 text-txt-4" />
          <div className="text-[14px] font-medium text-txt-3">暂无目标</div>
          <div className="text-[12px] text-txt-4 mt-1">点击上方按钮创建第一个目标</div>
        </div>
      ) : (
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? '编辑目标' : '创建目标'}
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-[12px] font-medium text-txt-3 mb-1.5">目标标题 <span className="text-red-400">*</span></label>
            <input value={form.title} onChange={e => setForm({...form, title: e.target.value})} className={inputCls} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[12px] font-medium text-txt-3 mb-1.5">层级</label>
              <select value={form.level} onChange={e => setForm({...form, level: e.target.value as 'company' | 'department' | 'individual'})} className={inputCls}>
            ...
            <div>
              <label className="block text-[12px] font-medium text-txt-3 mb-1.5">负责人</label>
              <input value={form.owner} onChange={e => setForm({...form, owner: e.target.value})} className={inputCls} />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label className="block text-[12px] font-medium text-txt-3 mb-1.5">目标值</label>
              <input type="number" value={form.target_value} onChange={e => setForm({...form, target_value: Number(e.target.value)})} className={inputCls} />
            </div>
            <div>
              <label className="block text-[12px] font-medium text-txt-3 mb-1.5">当前值</label>
              <input type="number" value={form.current_value} onChange={e => setForm({...form, current_value: Number(e.target.value)})} className={inputCls} />
            </div>
            <div>
              <label className="block text-[12px] font-medium text-txt-3 mb-1.5">单位</label>
              <input value={form.unit} onChange={e => setForm({...form, unit: e.target.value})} placeholder="万元" className={inputCls} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[12px] font-medium text-txt-3 mb-1.5">截止日期</label>
              <input type="date" value={form.deadline} onChange={e => setForm({...form, deadline: e.target.value})} className={inputCls} />
            </div>
            <div>
              <label className="block text-[12px] font-medium text-txt-3 mb-1.5">上级目标</label>
