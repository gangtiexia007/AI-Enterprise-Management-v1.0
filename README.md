...
- **ERP 订单导入**：每天上传 Excel，自动解析 28 个字段、提取运营归属、货币换算（PHP/MYR/THB→CNY）、状态分类
- **ERP 产品导入**：每天上传产品 Excel，自动匹配订单计算选品命中率
- **赛道自动分类**：关键词映射表（60% 覆盖）+ AI 补分类，产品标题自动归入赛道
- **SKU 自动分级**：基于出单量自动判定 S/A/有效链接/C 级
- **运营 KPI 排名**：有效链接数、爆款数、出单量、销售额(CNY)、取消率、命中率，按国家/平台拆分
- **KPI 自动桥接**：聚合数据自动写入 KPIRecord，激活现有辅导/告警/评分链路
